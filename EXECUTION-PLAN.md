...
- [~] **Ф4. Перенос экранов** — в работе, разбита на 4a/4b:
  - [x] **Ф4a. Фундамент** — NativeWind v4 (tailwind 3.4 + metro/babel) + тема из старых Colors; React Navigation v7 (root auth-gate + bottom-tabs); auth-слайс (isAuthenticated); UI-примитивы (Button/Input/Card/Screen); 4 стаб-экрана (Auth/Order/Orders/Profile) с реальными RTK Query хуками на mock. gate: typecheck/lint/format/build (3.5M) зелёные. ✅ чекпойнт
  - [ ] **Ф4b. Перенос экранов** — ~14 оставшихся экранов (Invoice/Balans, PaymentMethod, AddCard→безопасный, PromoKod→серверная валидация, Ratings, DriverOrder, CountryPicker, News, Gain, Modal, Radio) Flow→TS на примитивы + RTK Query; детальные стек-экраны в RootNavigator. **Fan-out через Workflow** (1 агент на экран). gate: навигация ходит по всем экранам на mock.
  - _UI-kit: gluestack-ui отложен (ADR-001) — NativeWind + свои примитивы._
...
- **Точка возобновления:** следующая — **Ф4b** (перенос ~14 оставшихся экранов Flow→TS на примитивы+RTK Query + детальные стек-экраны; fan-out через Workflow). Старые экраны: `~/Documents/mobile/taxi-app/App/Containers/*`.
